...
def mse(predictions, targets):
    ...
    """
    #print("prediction: " + str(predictions.shape))
    diff = predictions - targets
    loss_per_sample_and_seq = (diff * diff).sum(dim=-1)  # (N, F)
    #print("loss_per_sample" + str(loss_per_sample_and_seq.shape))
    #print("mean :" +str(loss_per_sample_and_seq.mean()))
...
def loss_pose_joint_sum(predictions, targets):
    ...
    """
    diff = predictions - targets
    per_joint_loss = (diff * diff).view(-1, 143, 15, 9)
